...
![image](https://minios.strongsickcat.com/dinghuang-blog-picture/1b24c0df.png)
...
![image](https://minios.strongsickcat.com/dinghuang-blog-picture/8aff3f22.png)
- 切分策略
    - 查询切分：将ID和库的Mapping关系记录在一个单独的库中。优点：ID和库的Mapping算法可以随意更改。缺点：引入额外的单点。
    ![image](https://minios.strongsickcat.com/dinghuang-blog-picture/ddb1ceb8.png)
    - 范围切分：比如按照时间区间或ID区间来切分。优点：单表大小可控，天然水平扩展。缺点：无法解决集中写入瓶颈的问题。
    ![image](https://minios.strongsickcat.com/dinghuang-blog-picture/f80cafa2.png)
    ...
    ![image](https://minios.strongsickcat.com/dinghuang-blog-picture/52610d6d.png)
        - 场景一：数据库性能达到瓶颈
            - 方法一：按照现有规则不变，可以直接扩展到32个数据库集群。
            ![image](https://minios.strongsickcat.com/dinghuang-blog-picture/b301cb95.png)
            - 方法二：如果32个集群也无法满足需求，那么将分库分表规则调整为(32*2^n)*(32⁄2^n)，可以达到最多1024个集群。
            ![image](https://minios.strongsickcat.com/dinghuang-blog-picture/c573941e.png)
        - 场景二：单表容量达到瓶颈（或者1024已经无法满足你）
            ...
            ![image](https://minios.strongsickcat.com/dinghuang-blog-picture/494ebd9e.png)
...
- 数据迁移
    ...
    - 第一阶段
    ![image](https://minios.strongsickcat.com/dinghuang-blog-picture/2ae1ba98.png)
        - 数据库双写（事务成功以老模型为准），查询走老模型。
        - 每日job数据对账（通过DW），并将差异补平。
        - 通过job导历史数据。
    - 第二阶段
    ![image](https://minios.strongsickcat.com/dinghuang-blog-picture/0a93304e.png)
        - 历史数据导入完毕并且数据对账无误。
        - 依然是数据库双写，但是事务成功与否以新模型为准，在线查询切新模型。
        - 每日job数据对账，将差异补平。
    - 第三阶段
        ![image](https://minios.strongsickcat.com/dinghuang-blog-picture/a8d54b30.png)
...
Mysql复制的流程图如下：
![image](https://minios.strongsickcat.com/dinghuang-blog-picture/907596-20170106160057409-885292032.png)
...
- 存储引擎
    - InnoDB
![image](https://minios.strongsickcat.com/dinghuang-blog-picture/1343758042_8526.png)
    ...
    - MyISAM
![image](https://minios.strongsickcat.com/dinghuang-blog-picture/1343757655_1008.png)
...
![image](https://minios.strongsickcat.com/dinghuang-blog-picture/20190708225116942.png)
...
![image](https://minios.strongsickcat.com/dinghuang-blog-picture/20190708225333630.png)
...
![image](https://minios.strongsickcat.com/dinghuang-blog-picture/20190708225423934.png)
...
![image](https://minios.strongsickcat.com/dinghuang-blog-picture/20190708225728222.png)

假设先写数据库，再淘汰缓存：第一步写数据库操作成功，第二步淘汰缓存失败，则会出现DB中是新数据，Cache中是旧数据，数据不一致。

![image](https://minios.strongsickcat.com/dinghuang-blog-picture/20190708225736738.png)
...
#### 缓存架构优化
![image](https://minios.strongsickcat.com/dinghuang-blog-picture/20190708225756637.png)

上述缓存架构有一个缺点：业务方需要同时关注缓存与DB，主要有两种优化方案：

![image](https://minios.strongsickcat.com/dinghuang-blog-picture/2019070822580935.png)

一种方案是服务化：加入一个服务层，向上游提供帅气的数据访问接口，向上游屏蔽底层数据存储的细节，这样业务线不需要关注数据是来自于cache还是DB。
![image](https://minios.strongsickcat.com/dinghuang-blog-picture/20190708225823969.png)
...
![image](https://minios.strongsickcat.com/dinghuang-blog-picture/20190708225900770.png)
...
写入缓存：
![image](https://minios.strongsickcat.com/dinghuang-blog-picture/20190708230031559.png)
- 应用同时更新数据库和缓存
...
![image](https://minios.strongsickcat.com/dinghuang-blog-picture/20190708230102585.png)

##### 客户端数据库与缓存解耦
上述方案对于应用的研发人员来讲比较重，需要研发人员同时考虑数据库和Redis是否成功来做不同方案，如何让研发人员只关注数据库层面，而不用关心缓存层呢？请看下图：
![image](https://minios.strongsickcat.com/dinghuang-blog-picture/20190708230139385.png)
...
![image](https://minios.strongsickcat.com/dinghuang-blog-picture/1.JPG)
...
![image](https://minios.strongsickcat.com/dinghuang-blog-picture/2.JPG)

但 二叉搜索树在经过多次插入与删除后，有可能导致不同的结构：

![image](https://minios.strongsickcat.com/dinghuang-blog-picture/31.JPG)
...
如：（M=3）
![image](https://minios.strongsickcat.com/dinghuang-blog-picture/4.JPG)
...
![image](https://minios.strongsickcat.com/dinghuang-blog-picture/0.JPG)
...
6.所有关键字都在叶子结点出现；
![image](https://minios.strongsickcat.com/dinghuang-blog-picture/5.JPG)
...
![image](https://minios.strongsickcat.com/dinghuang-blog-picture/6.JPG)
...
![image](https://minios.strongsickcat.com/dinghuang-blog-picture/20121130111257402.png)

这里设表一共有三列，假设我们以Col1为主键，则上图是一个MyISAM表的主索引（Primary key）示意。可以看出MyISAM的索引文件仅仅保存数据记录的地址。在MyISAM中，主索引和辅助索引（Secondary key）在结构上没有任何区别，只是主索引要求key是唯一的，而辅助索引的key可以重复。如果我们在Col2上建立一个辅助索引，则此索引的结构如下图所示：

![image](https://minios.strongsickcat.com/dinghuang-blog-picture/20121130111258238.png)
...
![image](https://minios.strongsickcat.com/dinghuang-blog-picture/20121130111259913.png)
...
![image](https://minios.strongsickcat.com/dinghuang-blog-picture/20121130111300494.png)
...
#### MYSQL 架构
![image](https://minios.strongsickcat.com/dinghuang-blog-picture/20190706105542781.png)
...
![image](https://minios.strongsickcat.com/dinghuang-blog-picture/20180727113023312.png)
...
可以汇总为如下表格：
![image](https://minios.strongsickcat.com/dinghuang-blog-picture/20180727113110216.png)
...
![image](https://minios.strongsickcat.com/dinghuang-blog-picture/20180727113601623.png)
![image](https://minios.strongsickcat.com/dinghuang-blog-picture/2018072711355127.png)
![image](https://minios.strongsickcat.com/dinghuang-blog-picture/2018072711351276.png)
...
![image](https://minios.strongsickcat.com/dinghuang-blog-picture/2604566-ad0d50c44dc041be.png)
...
![image](https://minios.strongsickcat.com/dinghuang-blog-picture/2604566-b1ddc3677c81c9b5.png)
...
![image](https://minios.strongsickcat.com/dinghuang-blog-picture/2604566-1f499d02104728e0.png)
...
![image](https://minios.strongsickcat.com/dinghuang-blog-picture/2604566-35dc3a0aed8a0f9a.png)
...
![image](https://minios.strongsickcat.com/dinghuang-blog-picture/2604566-3e7aaef9e3865926.png)
